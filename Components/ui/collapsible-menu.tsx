    ...
    <div className="mb-2 min-h-[48px] list-none last:mb-0">
      {dropdownItems?.length ? (
              ...
              {dropdownItems.map((item, index) => (
                <li className="first:pt-2" key={index}>
                  <ActiveLink
                    ...
                    className="flex items-center rounded-lg p-3 text-sm text-gray-500 transition-all before:h-1 before:w-1 before:rounded-full before:bg-gray-500  pl-6 before:mr-5  hover:text-white"
        ...
        <ActiveLink
          ...
          className={cn(
            "relative flex h-12 items-center whitespace-nowrap rounded-lg px-4 text-sm text-gray-500 transition-all hover:text-[#7c3aed]",
            {
              "bg-brand": isActive,
            }
          )}
          ...
          {href === pathname && (
            <motion.span
              className="absolute bottom-0 left-0 right-0 h-full w-full rounded-lg bg-[#7c3aed] shadow-large text-white"
              layoutId="menu-item-active-indicator"
            />
          )}
        </ActiveLink>
